...
class Client(object):
    ...
    def getAccessToken(self, refreshToken=None):
        ...
        url = self.baseUrl + 'DeviceLogin8?deviceKey='+self.device.key+'&advertisingKey=&isJailBroken=False&checksum='+ChecksumCreateDevice(self.device.key, self.device.name)+'&deviceType=DeviceType'+self.device.name+'&signal=False&languageKey=' + self.device.languageKey
        url += '&refreshToken='+(self.device.refreshToken if self.device.refreshToken else '')
    ...
    def login(self, email=None, password=None):
        ...
        checksum = ChecksumEmailAuthorize(self.device.key, email, ts, self.accessToken, self.salt)

        # if refreshToken was used we get acquire session without credentials
        if self.device.refreshToken:
            url = self.baseUrl + 'UserEmailPasswordAuthorize2?clientDateTime={}&checksum={}&deviceKey={}&accessToken={}&refreshToken={}'.format(
                ts,
                checksum,
                self.device.key,
                self.accessToken,
            ...
            url = self.baseUrl + 'UserEmailPasswordAuthorize2?clientDateTime={}&checksum={}&deviceKey={}&email={}&password={}&accessToken={}'.format(
                ts,
                checksum,
                self.device.key,
                email,
            ...
            if 'errorMessage=' in r.text:
                print('[login] failed to authorize with credentials with the reason:', r.text)
                return False
    ...
    def heartbeat(self):
        ...
        url = self.baseUrl + 'HeartBeat4?clientDateTime={}&checksum={}&accessToken={}'.format(
